...
動かしておきたい時間は分で書く。
1分から1440分（24時間）までで、この範囲を外れる値は保存の時点で断られる。
...
作られた Agent には、確定した作業内容（目的、説明、手順、確認したいこと、注意点）が1件目の指示として渡る。
Agent はこれを読んで最初の Tool を選ぶ。
...
終わった Task ごとに、図の再生と、起きたことの一覧が並ぶ。
...
図を再生すると、矢印の上にそのやり取りの名前が出て、関わった箱が光り、図の下の枠にどこからどこへ何をしたかの説明が出る。
同時に、いま説明している行が強調される。
