...
| RULE-03 | Agent Registration（issuer、subject、signing key、client credential、XAA config、expires_at）はIsolation Levelに関わらずAgentごとに作る | [05. §4](./05-identity.md#4-agent-registrationstandardでもagentごとに作る) |
...
| RULE-20 | Agent OPは判断しない。仕事はAgent認証、ID-JAG発行、KMS署名、期限確認のみ | [05. §3](./05-identity.md#3-agent-op) |
| RULE-21 | Native XAA ResourceではBridgeを使わない。BridgeはID-JAGを理解しない外部OAuth SaaS向けの互換レイヤーとしてのみ使う | [06. §1](./06-oauth-bridge.md#1-位置づけ) |
| RULE-22 | Refresh TokenとClient SecretをAgentへ渡さない。Agentに許すのは短期Access Tokenの取得とメモリ上での一時保持だけ | [05. §9](./05-identity.md#9-tokenの種類と保持ルール)、[06. §3](./06-oauth-bridge.md#3-credential保持方針) |
...
| RULE-37 | Internetへ公開するのはAutomation AppとGoogle BridgeのOAuth Callbackだけとする | [08. §8](./08-gcp-infrastructure.md#8-ネットワークと公開範囲) |
