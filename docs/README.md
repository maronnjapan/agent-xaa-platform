...
| 12 | [作業内容からの権限決定](./12-operation-authorization.md) | 作業を計画に分け、判定器で操作を選び、許可集合を Cross App Access の静的設定に写して Agent OP で強制する設計。カタログは OpenAPI 定義を出所にする |
...
- 2026-09-10：アクティビティの Agent のカードに「流れを通しで見る」を追加した（[11. §5.2](./11-activity-timeline.md#52-再生の中身)）。終わった区切りを起きた順につなぎ、ログインから終了までを1つの図で通して再生する。区切りごとの再生と同じ図・同じ間隔で動き、速さの2択と全画面の表示を持つ。図は箱を4つの帯に置いて帯に名前を書き、線が丸と一緒に伸びて届いた先に矢の先が付き、止められた往復の先を点線にし、同じイベントの前の往復を薄く残す。通しの再生は「登場人物」の紹介から始まり、辞書に日常の言葉でのたとえを加えた。
- 2026-09-10：アクティビティを「一覧」と「詳細画面」に分け、1画面に1つのことだけを出すようにした（[11. §5.1](./11-activity-timeline.md#51-画面の構成)）。一覧は Agent ごとのカードに区切りを1行ずつ並べるだけで、アニメーションとログはそれぞれ別のアドレスで詳細画面に開き、同じ場所で切り替える。アニメーションの説明は図の枠の中の字幕1か所に集め、Agent 自身の言葉と実行前の確認だけを添える（[11. §5.2](./11-activity-timeline.md#52-再生の中身)）。ログは区切りのできごとを1行ずつ並べ、選んだ1件だけを開く（[11. §5.4](./11-activity-timeline.md#54-できごとの一覧)）。
- 2026-09-19：作業内容から API の操作と対象の粒度で権限を決める設計を [12](./12-operation-authorization.md) に追加した。作業を計画（ステップの集合）に分け、選択と是非判定だけができる判定器でカタログの操作に当て、閾値とリスクで自動、確認、不付与に振り分け、作成者の権限との積を取ったうえで作成者が承認する。承認された許可集合は Agent OP の静的設定に `operations` として写し、ステップごとの ID-JAG にその操作と対象だけを載せて、Resource 側はトークンに書かれたことを検査するだけにする。カタログは OpenAPI 定義と拡張項目 `x-xaa` を出所にし、Tool ごとの `scope` は作らない。判定器は Jev を使うが配備の設定で Vertex AI 側に切り替えられる。
