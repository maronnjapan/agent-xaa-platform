...
- 2026-09-10：アクティビティを「一覧」と「ビューア」の2つの顔に分け、1画面に1つのことだけを出すようにした（[11. §5.1](./11-activity-timeline.md#51-画面の構成)）。一覧は Agent ごとのカードに区切りを1行ずつ並べるだけで、動きの図とできごとの記録はそれぞれ別のアドレスでビューアに開き、同じ場所で切り替える。記録は区切りのできごとを1行ずつ並べ、選んだ1件だけを開く（[11. §5.4](./11-activity-timeline.md#54-できごとの一覧)）。図の横の考えたことの枠は Agent 自身の言葉と実行前の確認に絞った（[11. §5.2](./11-activity-timeline.md#52-再生の中身)）。
