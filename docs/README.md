...
- 2026-09-10：アクティビティの Agent のカードに「流れを通しで見る」を追加した（[11. §5.2](./11-activity-timeline.md#52-再生の中身)）。終わった区切りを起きた順につなぎ、ログインから終了までを1つの図で通して再生する。区切りごとの再生と同じ図・同じ間隔で動き、速さの2択と全画面の表示を持つ。
