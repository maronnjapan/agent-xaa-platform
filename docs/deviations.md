...
|---|---|---|---|---|
| **DEV-01** | ドラフト ID-JAG は DPoP を要求しない。RULE-06 はこれを必須にする | `packages/xaa-crypto/src/dpop.ts` / `packages/xaa-crypto/src/dpop-middleware.ts` | `packages/xaa-crypto/test/dpop.spec.ts::rejects htm mismatch / normalizes htu / rejects replayed jti` | ドラフトだけを実装した Resource AS は Proof を無視するため、この platform の Access Token を安全に扱えない |
