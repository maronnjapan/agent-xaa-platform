...
| **DEV-13** | Firestore Security Rules を作らず、ブラウザからの直接アクセス禁止をビルド成果物の検査で担保する。DEC-IAC-10 | `infra/tests/no-firestore-sdk-in-frontend.sh` / `scripts/checks/no-persistent-connection.sh` | `apps/automation-app/test/frontend-boundaries.spec.ts::carries no datastore SDK and holds no connection open` | Security Rules を前提にした監査は、この禁止を確認できない |
