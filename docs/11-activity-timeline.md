...
- 最初の区切りは「登場人物」である。図に出る箱を1つずつ、物語の順（人、決める側、動く側、データを持つ側）に光らせ、他の箱を薄くし、箱の下に名前とたとえ、図の下に一行の役割・すること・しないこと、図の横にその箱の説明を出す。再生を押す前は、図の横に登場人物の一覧（名前・たとえ・役割・すること・しないこと）を出す。どれも上の辞書の固定文であり、イベントの解釈ではない（RULE-54）。紹介を飛ばすには次の区切りを押す。
