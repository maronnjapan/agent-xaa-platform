...
アクティビティ画面は、**一覧**と、1つのAgentを開く**ビューア**の2つの顔を持つ。どちらも同じ`/activity`であり、アドレスの`run`（Agentのまとまり）、`task`（区切り）、`view`（`replay`か`log`）、`event`（できごと）が、どちらの顔をどこで開くかを決める。サーバーはアドレスが指す顔をそのまま描画するので、スクリプトの無いブラウザでもリンクをたどれば区切りの記録が読め、スクリプトがあれば同じページに操作が付く。

1. **一覧**：ユーザーの全Agentについて、Agentごとに1枚のカードにまとめ、新しく始めたAgentを上に置く。カードの頭には、Agentの目的、開始と最後の記録の時刻、Taskの数、実行中のTaskの数、遮断を含むTaskの数、失敗を含むTaskの数、終了したかどうかを出す。どれもイベントの`outcome`と`task_id`を数えたものであり、画面が結論を作ることはない（RULE-54）。Agentがまだ無い物語は「Agentはまだ作られていません」と添える。Agentの画面へのリンクと、そのAgentだけに絞る操作もここに置く。終わった区切りが1つでもあるAgentには、その物語を通しで再生するビューアへのリンク「流れを通しで見る」（[§5.2](#52-再生の中身)）を置く。`agent_id`のような識別子は「ID を表示」に畳み、頭には出さない。画面の先頭には、Agentの数・実行中の数・遮断のあった数・失敗のあった数をまとめて出す。
   カードの中では、1つのAgentのTaskを起きた順（`provisioning`、完了順の`task-{n}`、`lifecycle`）に、縦の線に沿って1行ずつ並べる。呼び名は`task_id`そのものではなく「準備」「作業 1」「終了」「デモ」とし、各行にはそのTaskの最後のイベントの`title`、終端の`outcome`、イベント1件につき1つの点（`phase`と`outcome`から色を決める）、完了時刻、所要時間、イベントの件数、そのうち遮断された件数と失敗した件数、そしてビューアへの2つのリンク「動きを図で見る」「できごとを読む」を出す。実行中の`task_id`は「実行中」として名前だけ出し、リンクを持たない。**一覧の上では何も動かず、何も開かない。** 区切りを開くと一覧と図がその場で広がる形は、Agent 1体で区切りの数だけ図と文章が並び、上には通しの再生まで重なって、何をしているのか目で追えなかった。
2. **ビューア**：1つのAgentについて、画面に1つのことだけを出す。頭にAgentの目的、一覧へ戻るリンク、「動きを図で見る」「できごとを読む」の切り替えを置き、その下に区切りの並び（「登場人物」「準備」「作業 1」「終了」。実行中の区切りは押せない印として出す）と、いまの区切りの呼び名と最後のイベントの`title`を1行で出す。**動きを図で見る**は[§5.2](#52-再生の中身)の図と操作と、図の横の1つの枠（再生前は登場人物の一覧、紹介中はその登場人物、再生中は考えたこと、箱を押したときはその箱の説明）である。**できごとを読む**は[§5.4](#54-できごとの一覧)であり、いまの区切りのできごとを1行ずつ左に、選んだ1件の記録だけを右に出す。狭い画面では右のものが下に来る。
   2つの顔は「いま物語のどの手にいるか」を1つの状態として共有する。図が止まっている手は、記録で選ばれている行であり、記録で別の行を選ぶと図はその手で止まる。区切りを押すとその区切りの先頭へ移る。顔を切り替えても場所は変わらない。アドレスも追従するので、読み込み直しても共有しても、同じ顔・同じ区切り・同じできごとで開く。

図と記録は代替ではない。図は「何と何が話し、どこで止まったか」に答え、記録は「何を送り、何が返り、先に何を確かめたか」に答える。片方だけでは、もう片方の問いが残る。だが同時に見せると目がどちらも追えないので、同じ場所で顔を切り替える。アニメーションを見られない人にとっても、失われるのは動きだけである。
...
「作業 2」は、ユーザーが自分から権限外の指示を試した結果であり、実際にTool Executorが下した拒否である（[§6.1](#61-実操作で見せる)）。各行の見出しはそのTaskの最後のイベントの`title`であり、画面が書いたものではない。行の「できごとを読む」で[§5.4](#54-できごとの一覧)の記録が、「動きを図で見る」で[§5.2](#52-再生の中身)の再生が、それぞれビューアに開く。
...
**絵の横に、その手で何を考えたかを短く出す。** 図が答えるのは「何がどこへ行ったか」だけで、人が実際に聞きたい「AIエージェントが何を考えて、どうすると決めたのか」には答えていない。その答えは[§3.4](#34-activity-record)のRecordに最初から入っているが、要求本文やTokenの有効期限と同じ形で畳まれていて、絵が動いている間は誰も開かない。そこで再生中の1手について、Recordから次の2つだけを図の横に出す。

| 区分 | 何を出すか | どこから取るか |
|---|---|---|
| 考えたこと | モデル自身の言葉、AIが述べた根拠、人が書いた指示 | 発行元が`format: text`と印を付けた`sections`（引用として出す） |
| 確かめたこと | 実行前の検査とその判定 | `checks` |

読み取ったもの、選んだToolと引数、要求と応答の本文は、ここには出さない。動いている絵の横で読めるのはこの2つまでで、それ以上を置くと絵と文の両方を目で追えなくなる。残りは同じ枠の末尾の「このできごとの記録を読む」で開く記録（[§5.4](#54-できごとの一覧)）にあり、リンクは同じできごとの上で開く。仕分けの根拠は、発行元が「本文を散文と宣言したかどうか」だけである。見出しは画面のものだが、文は1つも画面が作らない（RULE-54、REQ-11-002）。1件のイベントが4つのhopsに展開されるとき、この枠はそのイベントの間ずっと同じものを出し続ける。考えたのはhopではなくイベントだからである。

記録の側では、再生を見る前後どちらでも[§3.1](#31-スキーマ)の`detail`を「技術的な詳細」として開けるようにし、技術的な内容はそこで確認できるようにする。`detail`の鍵のうち画面が知っているもの（`decision_id`、`tool_id`など）には固定の日本語の見出しを添え、鍵そのものも残す。値は届いたまま出す。

**区切りをまたいで通しで見る。** 1つの区切りの再生は、その区切りで何が起きたかに答える。だが人に見せるときに要るのは、ログインから、権限の決定、Agentの作成、Toolの呼び出し、終了までが1本につながって動く絵であり、区切りを4つ開いて再生を4回押す形では見せられない。そこでビューアの再生は、そのAgentの終わった区切りをすべて起きた順につないだ1本の計画で動く。Agentのカードの頭の「流れを通しで見る」はその先頭から、区切りの行の「動きを図で見る」はその区切りから開き、再生はそこから区切りをまたいで進む。図、矢印のそばの`label`、図の下の枠、考えたことの枠、1ステップの長さは共通であり、通しで見るために足したものは次の4つである。

- 最初の区切りは「登場人物」である。図に出る箱を1つずつ、物語の順（人、決める側、動く側、データを持つ側）に光らせ、他の箱を薄くし、箱の下に名前とたとえ、図の下に一行の役割・すること・しないこと、図の横にその箱の説明を出す。再生を押す前は、図の横に登場人物の一覧（名前・たとえ・役割を1行ずつ）を出す。すること・しないことは紹介のときと、箱を押したときに出す。どれも上の辞書の固定文であり、イベントの解釈ではない（RULE-54）。紹介を飛ばすには次の区切りを押す。
- 箱は、どれかの区切りに登場したものをすべて出し、区切りが変わっても消さない。区切りごとに箱が現れたり消えたりすると、見ている人は景色の変化をできごとと読む。
- 図の上に区切りの並びを置く。各区切りは呼び名（「準備」「作業 1」「終了」「デモ」）と終端の`outcome`を持ち、いまの区切りを光らせ、通り過ぎた区切りを薄くする。区切りを押すと、そこから再生する。並びの直下には、いまの区切りの呼び名、その区切りの最後のイベントの`title`、その区切りの中で何手目かを1行で出す。手数は区切りの中で数える。図の下の枠と進み具合の帯も同じ数え方をする。
- 「速さ」を「ふつう」「速く」から選べる。「速く」は1ステップと動きをともに半分にする。上で決めた長さは添えた文を読むための長さであり、人前で話しながら見せるときは待ち時間になるためである。2倍より速くはしない。それ以上では丸の動きが「箱から箱へ移った」と読めない。「全画面で見る」は、ビューアを画面いっぱいに出す。

ビューアは、押すまで動き出さず、繰り返さない。デモ専用の画面ではなく、同じ画面の同じ記録を、まとめて動かしているだけである（[§1](#1-位置づけ)）。台本の区切りが含まれるときは、その区切りに「デモ実行（模擬）」の表示が付いたまま再生される（[§5.3](#53-表示のルール)）。
...
ビューアの「できごとを読む」は、いまの区切りのイベントを1行ずつ左に並べ、選んだ1件の記録を右に出す（[§5.1](#51-画面の構成)）。1行には、何番目か、発生元の箱の日本語の名前、`phase`の呼び名（ログイン、ToDo、権限の決定、Agent の作成、ツールの実行、セキュリティ、終了）、時刻、前の行からの経過、`outcome`のバッジ、`title`を置く。行の左端には縦の線が通り、行ごとに`phase`の記号を結果の色の丸で並べるので、長い記録でも止められた行を探せる。行はそのできごとの記録へのリンクであり、スクリプトが無ければリンクをたどって、あればその場で右側が入れ替わる。

右側には、その箱が何をするところかの一行、`title`、`message`、`hops`を箱と矢印で並べた経路、そして[§3.4](#34-activity-record)のRecordを置く。名前と一行はどちらも§5.2の辞書から取り、図と同じ言葉で呼ぶ。`phase`の呼び名は固定の7語であり、値の呼び名であって解釈ではない。Recordのうち`checks`は最初から開いておく。「何かに止められたのか」への答えであり、開かないと分からない形にすると読まれないからである。発行元が`format: text`と印を付けた文章——Agentが自分で書いた考え、Authorization AIが述べた根拠、人が書いた指示——も開いたまま引用として出す。読む人が最初に知りたいのは「何を考えていたか」だからである。技術的な値（要求、応答、Token の有効期限など）は畳んでおく。記録を出すのは選んだ1件だけである。10行の下に10件の記録を開いた形は、誰も読めない壁だった。
