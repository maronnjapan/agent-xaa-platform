...
アクティビティ画面は、**一覧**と、1つのAgentを開く**詳細画面**からなる。どちらも同じ`/activity`であり、アドレスの`run`（Agentのまとまり）、`task`（区切り）、`view`（`replay`か`log`）、`event`（できごと）が、どちらの画面をどこで開くかを決める。サーバーはアドレスが指す画面をそのまま描画するので、スクリプトの無いブラウザでもリンクをたどれば区切りの記録が読める。スクリプトがあれば、同じページに操作が付く。

1. **一覧**：ユーザーの全Agentについて、Agentごとに1枚のカードにまとめ、新しく始めたAgentを上に置く。カードの頭には、Agentの目的、開始と最後の記録の時刻、Taskの数、実行中のTaskの数、遮断を含むTaskの数、失敗を含むTaskの数、終了したかどうかを出す。どれもイベントの`outcome`と`task_id`を数えたものであり、画面が結論を作ることはない（RULE-54）。Agentがまだ無い物語は「Agentはまだ作られていません」と添える。Agentの画面へのリンクと、そのAgentだけに絞る操作もここに置く。終わった区切りが1つでもあるAgentには、物語を最初から通して再生する詳細画面へのリンク「最初から通して見る」（[§5.2](#52-再生の中身)）を置く。`agent_id`のような識別子は「ID を表示」に畳み、頭には出さない。画面の先頭には、Agentの数、実行中の数、遮断のあった数、失敗のあった数をまとめて出す。
   カードの中では、1つのAgentのTaskを起きた順（`provisioning`、完了順の`task-{n}`、`lifecycle`）に、縦の線に沿って1行ずつ並べる。呼び名は`task_id`そのものではなく「準備」「作業 1」「終了」「デモ」とし、各行にはそのTaskの最後のイベントの`title`、終端の`outcome`、イベント1件につき1つの点（`phase`と`outcome`から色を決める）、完了時刻、所要時間、イベントの件数、そのうち遮断された件数と失敗した件数、そして詳細画面への2つのリンク「アニメーションで見る」「ログを読む」を出す。実行中の`task_id`は「実行中」として名前だけ出し、リンクを持たない。一覧の上では何も動かず、何も開かない。区切りを開くとその場に一覧と図が広がる形では、Agent 1体につき区切りの数だけ図と文章が並び、その上に通しの再生まで重なって、何をしているのか目で追えなかったからである。
2. **詳細画面**：1つのAgentについて、画面に1つのことだけを出す。頭にAgentの目的、一覧へ戻るリンク、「アニメーション」「ログ」の切り替えを置き、その下に区切りの並び（「登場人物」「準備」「作業 1」「終了」。実行中の区切りは押せない印として出す）を出す。**アニメーション**は[§5.2](#52-再生の中身)の図と操作であり、いまの手についての説明はすべて図の枠の中に書く。図の横や下に別の説明枠は置かない。**ログ**は[§5.4](#54-できごとの一覧)であり、いまの区切りのできごとを1行ずつ左に、選んだ1件の記録だけを右に出す。狭い画面では右のものが下に来る。
   2つの表示は「いま物語のどの手にいるか」を1つの状態として共有する。図が止まっている手は、ログで選ばれている行であり、ログで別の行を選ぶと図はその手で止まる。区切りを押すとその区切りの先頭へ移る。表示を切り替えても場所は変わらない。アドレスも追従するので、読み込み直しても共有しても、同じ表示、同じ区切り、同じできごとで開く。

図とログは代替ではない。図は「何と何が話し、どこで止まったか」に答え、ログは「何を送り、何が返り、先に何を確かめたか」に答える。片方だけでは、もう片方の問いが残る。だが同時に見せると目がどちらも追えないので、同じ場所で切り替える。アニメーションを見られない人にとっても、失われるのは動きだけである。

**問題があったものだけを見る切り替え**を一覧の先頭に置く。押すと`outcome`が`blocked`でも`failed`でもないTaskの行がスタイルで隠れ、該当の無いAgentのカードには「遮断や失敗はありません」とだけ出る。行を消すのではなく隠すだけなので、スクリプトの無い環境では全部が見える。検索欄は目的・`agent_id`・Taskの呼び名で Agent のカードを絞り、同じく隠すだけである。
...
「作業 2」は、ユーザーが自分から権限外の指示を試した結果であり、実際にTool Executorが下した拒否である（[§6.1](#61-実操作で見せる)）。各行の見出しはそのTaskの最後のイベントの`title`であり、画面が書いたものではない。行の「ログを読む」で[§5.4](#54-できごとの一覧)の記録が、「アニメーションで見る」で[§5.2](#52-再生の中身)の再生が、それぞれ詳細画面に開く。
...
**箱の名前だけでは何をするところか分からない。** `agent-op`や`resource-as`という名前は、docs 05を読んだ人にしか読めない。そこで登場するもの1つずつについて、画面が呼ぶ日本語の名前（「Agent の身元発行」「リソース認可」）・正式名（Agent OP、Resource AS）・一行の役割・日常の言葉でのたとえ（「身分証の窓口」「審査係」）・**すること**・**しないこと**の6つを固定文として持ち、その記録に出てきたものだけを一覧として画面の先頭に畳んで置く。図の箱は「人」「決める側」「動く側」「データを持つ側」の4つの帯の上に置き、帯の左端にその名前を書く。上の段が決める側で下の段が動く側だという凡例を、図そのものが言うためである。どの帯にも出ている箱が無いときは、帯も出さない。図の箱、一覧の各行、図の下の枠はどれも日本語の名前で呼び、正式名は説明の中と、名前に重ねた注記に残す。図の箱を押しても同じ説明が出る。「しないこと」を必ず書くのは、この基盤の要点が「決める側と動く側が別である」ことであり、それを知らない読み手には矢印が手前で止まる意味が分からないからである。この辞書は図・一覧・説明の3か所が同じ名前を使うための単一の出所でもある。これらも凡例であって、個々のイベントの解釈ではない（RULE-54）。

**1回のTool呼び出しは1本の矢印ではない。** Agent OPがID-JAGを発行し、Resource ASがそれをAccess Tokenに換え、Resource APIが答える——実際には4往復である。1本の矢印で描くと「Agentが何かに触った」しか言えず、遮断がどこで起きたのかも示せない。そこで発行元が経路を`hops`（[§3.4](#34-activity-record)）として記録し、再生はその1本ずつを順に動かす。経路を知っているのは呼び出した本人であり、画面ではない。同じイベントの中では、前の往復を線だけ薄く残す。4本目の矢印が動くとき先の3本が薄く見えていれば、それが1回の呼び出しの4往復だと絵で分かる。残すのは線だけで、文字と丸は残さない。

**動きには言葉を添える。** 丸が動くだけでは「何かがどこかへ行った」としか分からない。矢印のそばにはその往復の`label`を描き、関わった2つの箱を光らせ、図の枠の中の下段（映像の字幕にあたる場所）に「どこからどこへ」「何という往復か」「発行元の`message`」を出す。説明を置く場所はこの1か所だけである。図の横に説明の枠を置いていたときは、動く絵と横の文章のどちらを見ればよいか定まらなかった。どれも発行元が書いた文字列と箱の固定の見出しを置くだけで、ブラウザが文を作ることはない（RULE-54）。`hops`も宛先も持たないイベント——権限の決定や登録のように、1つの箱の中で起きたこと——は、矢印を描かずにその箱を光らせる。以前はこうしたイベントを、直前の箱へ戻る矢印として描いていたが、それは起きていない呼び出しを描くことだった。線は丸と一緒に伸び、丸が着いた先に矢の先を付ける。止められた往復は、止まった位置までを実線、その先の届かなかった部分を点線で描き、矢の先は付けない。図の下の`message`は、丸が着いた時点で出す。動いている間に出すと、まだ起きていないことを先に言うことになる。
...
2. 1件ずつ、発生元から宛先へ向かう動きを表示し、到達した時点でその`message`を示す。同時に、[§5.4](#54-できごとの一覧)の対応する行を強調する。図に出すのは、いま動いている1件だけである。前の1件が描いた矢印・`label`・`message`は次へ進む前に消す。全部を残すと、絵は「いま何が起きているか」に、それまでに起きたこと全部を重ねて答えることになる。起きたことの全部は[§5.4](#54-できごとの一覧)に順番どおり残る。例外は同じイベントの中の前の往復で、線だけを薄く残す（上記）。
...
**字幕には、その手で何を考えたかも短く添える。** 図が答えるのは「何がどこへ行ったか」だけで、人が実際に聞きたい「AIエージェントが何を考えて、どうすると決めたのか」には答えていない。その答えは[§3.4](#34-activity-record)のRecordに最初から入っているが、要求本文やTokenの有効期限と同じ形で畳まれていて、絵が動いている間は誰も開かない。そこで再生中の1手について、Recordから次の2つだけを`message`の下に続ける。

| 区分 | 何を出すか | どこから取るか |
|---|---|---|
| Agentの言葉 | モデル自身の言葉、AIが述べた根拠 | 発行元が`format: text`と印を付けた`sections`のうち、`received`と`work_definition`以外（発行元の`label`を添え、引用として出す） |
| 実行前の確認 | 実行前の検査とその判定 | `checks`（`label`と判定だけ） |

読み取った指示、選んだToolと引数、要求と応答の本文、確認の理由文は、字幕には出さない。動く絵と一緒に読めるのはこの2つまでで、それ以上を置くと絵と文の両方を目で追えなくなる。残りはログ（[§5.4](#54-できごとの一覧)）にあり、同じ手のまま「ログ」に切り替えれば同じできごとの記録が開く。仕分けの根拠は、発行元が書いた`id`と「本文を散文と宣言したかどうか」の2つだけである。見出しは画面のものだが、文は1つも画面が作らない（RULE-54、REQ-11-002）。1件のイベントが4つのhopsに展開されるとき、この2つはそのイベントの間ずっと同じものを出し続ける。考えたのはhopではなくイベントだからである。

ログの側では、再生を見る前後どちらでも[§3.1](#31-スキーマ)の`detail`を「技術的な詳細」として開けるようにし、技術的な内容はそこで確認できるようにする。`detail`の鍵のうち画面が知っているもの（`decision_id`、`tool_id`など）には固定の日本語の見出しを添え、鍵そのものも残す。値は届いたまま出す。

**区切りをまたいで通しで見る。** 1つの区切りの再生は、その区切りで何が起きたかに答える。だが人に見せるときに要るのは、ログインから、権限の決定、Agentの作成、Toolの呼び出し、終了までが1本につながって動く絵であり、区切りを4つ開いて再生を4回押す形では見せられない。そこで詳細画面の再生は、そのAgentの終わった区切りをすべて起きた順につないだ1本の計画で動く。Agentのカードの頭の「最初から通して見る」はその先頭から、区切りの行の「アニメーションで見る」はその区切りから開き、再生はそこから区切りをまたいで進む。図、矢印のそばの`label`、字幕、1ステップの長さは共通であり、通しで見るために足したものは次の4つである。

- 最初の区切りは「登場人物」である。図に出る箱を1つずつ、物語の順（人、決める側、動く側、データを持つ側）に光らせ、他の箱を薄くし、箱の下に名前とたとえ、字幕に一行の役割、すること、しないことを出す。どれも上の辞書の固定文であり、イベントの解釈ではない（RULE-54）。紹介を飛ばすには次の区切りを押す。箱を押したときも、同じ辞書の説明が字幕の場所に出る。
- 箱は、どれかの区切りに登場したものをすべて出し、区切りが変わっても消さない。区切りごとに箱が現れたり消えたりすると、見ている人は景色の変化をできごとと読む。
- 図の上に区切りの並びを置く。各区切りは呼び名（「準備」「作業 1」「終了」「デモ」）と終端の`outcome`を持ち、いまの区切りを光らせ、通り過ぎた区切りを薄くする。区切りを押すと、そこから再生する。手数は区切りの中で数え、字幕と操作の列に「1 / 3」のように出す。
- 「速さ」を「ふつう」「速く」から選べる。「速く」は1ステップと動きをともに半分にする。上で決めた長さは添えた文を読むための長さであり、人前で話しながら見せるときは待ち時間になるためである。2倍より速くはしない。それ以上では丸の動きが「箱から箱へ移った」と読めない。「全画面」は、詳細画面を画面いっぱいに出す。

詳細画面は、押すまで動き出さず、繰り返さない。デモ専用の画面ではなく、同じ画面の同じ記録を、まとめて動かしているだけである（[§1](#1-位置づけ)）。台本の区切りが含まれるときは、その区切りに「デモ実行（模擬）」の表示が付いたまま再生される（[§5.3](#53-表示のルール)）。
...
詳細画面の「ログ」は、いまの区切りのイベントを1行ずつ左に並べ、選んだ1件の記録を右に出す（[§5.1](#51-画面の構成)）。1行には、何番目か、`title`、発生元の箱の日本語の名前、時刻、前の行からの経過、`outcome`のバッジを置く。行の左端には縦の線が通り、行ごとに`phase`の記号を結果の色の丸で並べるので、長い記録でも止められた行を探せる。`phase`の呼び名（ログイン、ToDo、権限の決定、Agent の作成、ツールの実行、セキュリティ、終了）は記号の注記と右側の記録に置き、行には並べない。行はそのできごとの記録へのリンクであり、スクリプトが無ければリンクをたどって、あればその場で右側が入れ替わる。

右側には、発生元の名前と、その箱が何をするところかの一行、`phase`の呼び名、時刻、`outcome`のバッジ、`title`、`message`、`hops`を箱と矢印で並べた経路、そして[§3.4](#34-activity-record)のRecordを置く。名前と一行はどちらも§5.2の辞書から取り、図と同じ言葉で呼ぶ。`phase`の呼び名は固定の7語であり、値の呼び名であって解釈ではない。Recordのうち`checks`は最初から開いておく。「何かに止められたのか」への答えであり、開かないと分からない形にすると読まれないからである。発行元が`format: text`と印を付けた文章（Agentが自分で書いた考え、Authorization AIが述べた根拠、人が書いた指示）も開いたまま引用として出す。読む人が最初に知りたいのは「何を考えていたか」だからである。技術的な値（要求、応答、Token の有効期限など）は畳んでおく。経路は箱と矢印で描いた1つだけで、Recordの`hops`の一覧は重ねて出さない。記録を出すのは選んだ1件だけである。10行の下に10件の記録を開いた形は、誰も読めない壁だった。

時刻は記録どおりの値（UTC）を`<time>`に持たせ、ブラウザが読む人の時計で表示し直す。並び順は変わらない。記録の値そのものは要素に残す。

この一覧と記録はサーバー側で描画する。ブラウザが行うのは、選んだ行に印を付けて右側を入れ替えることだけで、文章は書かない。ブラウザが一文でも作れば、それは立ち会っていない出来事についてブラウザが述べたことになる（RULE-54）。
