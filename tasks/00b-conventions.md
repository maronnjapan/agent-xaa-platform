...
| `- [~]` | live の GCP プロジェクト、`gcloud`、Docker daemon、実 Vertex AI のいずれかが無いと観測できない。行内に、デプロイ後にそれを観測する `infra/tests/` または `scripts/` 配下のスクリプトをバッククォートで1つ以上書く | 観測スクリプトが無い、または実在しない行があれば非 0 で終了する |
