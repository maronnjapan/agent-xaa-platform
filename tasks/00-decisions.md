...
- DEC-APP-06 Automation App の UI は React SPA にせず、Hono JSX の SSR と esbuild で束ねた vanilla TypeScript にする。タイムラインの再生は固定8ノードの静的 SVG に CSS アニメーションを重ね、1ステップ 800ms 固定にする。
  ...
  - 再改訂(2026-09-10)。**アクティビティを「一覧」と「ビューア」の2つの顔に分け、1画面に1つのことだけを出す。** 区切りを開くと一覧と図がその場で広がり、カードの頭に通しの再生も重なる形は、画面のあちこちで動きと文章が同時に出て、何をしているのか目で追えなかった。一覧のカードは区切りを1行ずつ並べるだけで、何も動かず何も開かない。行の「動きを図で見る」「できごとを読む」とカードの頭の「流れを通しで見る」は、同じ `/activity` に `run` / `task` / `view` / `event` を付けたアドレスであり（`ui/activity-links.ts`）、サーバーはアドレスが指す顔をそのまま描画する（`components/activity-viewer.tsx`）。スクリプトが無くてもリンクをたどれば読める点は変わらない。ビューアは1つの Agent について、図（と横の1つの枠：登場人物・紹介中の登場人物・考えたこと・押した箱の説明のいずれか）か、記録（区切りのできごとを1行ずつと、選んだ1件の記録）のどちらか一方だけを出し、両者は「いま何手目か」を1つの状態として共有する。手数は区切りの中で数える。考えたことの枠は Agent 自身の言葉と実行前の確認だけに絞り、残りは記録へのリンクで開く（`replay/thinking.ts`）。`components/run-player.tsx` / `task-replay.tsx` / `stage-card.tsx` は無くなり、枠の出入りは CSS が受け持って `motion` は進捗の帯にだけ残る。区切りの並び、登場人物の紹介、「問題があったものだけ」（行を隠すだけ）はそのままである。
