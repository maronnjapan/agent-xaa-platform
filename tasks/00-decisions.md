...
- DEC-IAC-06 seed の宛先解決は「Terraform が解決済みの値を成果物として書き出す」形にする。`google_storage_bucket_object "platform_endpoints"` に issuer / jwks_url / xaa_token_url / 各 Resource の URL / スロット一覧を jsonencode して非公開バケットへ置き、seed ジョブはそれを読む。Job の中で terraform コマンドを実行しない。
- DEC-IAC-07 FULL_ISOLATION は Terraform 事前作成スロットのリース方式。変数 dedicated_slot_count(既定 2、1〜4)で for_each し、スロットごとに Cloud Run Service / 専用 SA / 専用 ID-JAG 署名鍵 / 専用 IdP Connection 暗号鍵 / 専用 Cloud Run Job / IAM Binding を作る。**実行時に GCP リソースを作成・削除・更新するコードを書かない**(Cloud Run の env 書き換えも禁止)。スロット Service には Terraform が静的 env SLOT_INDEX のみを注入し、担当 Agent は Agent Registration の `dedicated_op_slot_index == SLOT_INDEX` で判定する。
- DEC-IAC-08 Provisioner / Lifecycle の SA に Cloud Run Admin / Service Account Admin / KMS 鍵作成の権限を与えない。付与するのは Job Execution の起動と取り消し、KMS の鍵バージョン追加と無効化、Firestore の読み書きに限る。`infra/tests/forbidden-roles.sh` で禁止ロールの不在を検査する。
- DEC-IAC-09 データストアは **Firestore(Native mode)1本**にする。Cloud SQL を既定構成では使わない。原子性が要るスロット払い出しと Provisioning Transaction は Firestore の runTransaction で行う。生成 OP のストアは templates.ts の JsonStoreBackend を Firestore バックエンドで実装して差し替える。docs 08 §7.1 の論理DB分離は「アプリ側のパスガード(許可マトリクス1ファイル)」で代替する。
...
## 費用
- DEC-COST-01 既定プロファイル（issuer_profile=direct、enable_google_bridge=false、スロット2、Cloud SQL なし）を起動したまま放置した場合の月額固定費は約 $0.5 とする。内訳は Cloud KMS が約 $0.30、Artifact Registry が約 $0.15、Secret Manager が約 $0.06 で、Cloud Run と Firestore と Pub/Sub と Cloud Scheduler と Cloud Logging と BigQuery は検証規模なら無料枠に収まる。8時間のデモ1回あたりの従量は Vertex AI が約 $0.7、Cloud Run が約 $0.4 で、1日あたり $1.1 から $1.5 に収まる。`make demo-destroy` 後の残存は約 $0.5/月。
- DEC-COST-02 費用に効く操作点を5つに固定し、すべて Terraform 変数として露出させる。`issuer_profile`、`enable_google_bridge`、`dedicated_slot_count`、`agent_max_lifetime_seconds`、`enable_lb_reservation`。
