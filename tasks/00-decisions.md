...
- DEC-APP-06 Automation App の UI は React SPA にせず、Hono JSX の SSR と esbuild で束ねた vanilla TypeScript にする。タイムラインの再生は固定8ノードの静的 SVG に CSS アニメーションを重ね、1ステップ 800ms 固定にする。
  - 改訂(2026-09-06)。**React にする。ただし SPA にはしない。** 画面は React コンポーネント1組で書き、サーバは `react-dom/server` で HTML を描き、ブラウザは同じ値から `hydrateRoot` で同じ木を描き直す。スクリプトが動かなくても記事は全部読める点は変わらず、押せるものだけが hydration で増える。束ねるのは esbuild のまま、出力は画面ごとではなく1本 `/app.js` にする（画面ごとに束ねると React が4回入る）。ページの入力値は `<script type="application/json" id="page-data">` に入れ、サーバとブラウザが同じ1つの値から描く。
  ...
  - 再改訂(2026-09-09)。**アクティビティは「Agent ごとのカード → Task の縦の並び → 開いた Task の中に一覧と図」の3層にする。** 「動きを見る」「やったこと」を Agent 単位でまとめて並べる形は、1つの Agent で画面が数枚になり、ID と英語の名前がそのまま並んで読めなかった。人が最初に知りたいのは「どの Agent が、いつ、どこまで進み、止められたか」なので、カードの頭にそれだけを出し、Task は `provisioning` / `task-1` / `lifecycle` ではなく「準備」「作業 1」「終了」と呼び、閉じた状態で1行にする。開いた Task では、できごとの一覧を左に、その動きの再生と考えたことの枠を右に固定して置き、いま説明している行を一覧側で強調する（`components/run-card.tsx`、`components/stage-card.tsx`）。ID は「ID を表示」の中に畳み、画面が呼ぶ名前は `roles.ts` の日本語名（`name`）と `labels.ts` の固定語に統一する。どれも値の呼び名であって、できごとの解釈ではない（RULE-54）。「遮断されたものだけ」の切り替えは、行を消すのではなくスタイルで隠すだけで、スクリプトが無ければ全部が見える。
  - 再改訂(2026-09-10)。**Agent のカードに「流れを通しで見る」を置き、終わった区切りを全部つないで1つの図で再生できるようにする。** 区切りごとの再生は「その区切りで何が起きたか」には答えるが、人に見せるときに要るのは、ログインから終了までが1本につながって動く絵であり、区切りを開いて再生を4回押す形では見せられない。押すとカードの頭と区切りの並びの間に枠が開き（`components/run-player.tsx`）、各区切りの再生計画を起きた順につないだ1本の計画（`replay/story.ts`）を、同じ図・同じ間隔・同じ考えたことの枠で流す。箱はどれかの区切りに出たものを全部出したままにし、図の上に区切りの並びを置いて、いま再生している区切りを光らせ、押した区切りから再生する。1手の位置は通しの再生が持ち、下の並びはそれを受け取って区切りと行に印を付けるだけである。「速さ」は「ふつう」と「速く」（1ステップと動きを半分）の2択で、それ以上は速くしない。「全画面で見る」はブラウザの全画面 API を呼ぶだけで、何も決めない。枠は押すまで描かず、サーバは描かない。デモ専用の画面ではなく、同じ画面の同じ記録である（RULE-60）。同日、絵だけで何が起きているか読めるように図を改めた。箱を4つの帯（人・決める側・動く側・データを持つ側）の上に置いて帯に名前を書き、線は丸と一緒に伸びて届いた先に矢の先が付き、止められた往復は止まった位置から先を点線にし、同じイベントの前の往復は線だけ薄く残す。`message` は丸が着いてから出す。通しの再生は「登場人物」の区切りから始め、図に出る箱を1つずつ光らせて名前・たとえ・役割・すること・しないことを出し、再生前は図の横に登場人物の一覧を出す。辞書（`roles.ts`）に日常の言葉でのたとえ（「審査係」「身分証の窓口」）を加えた。どれも固定文で、イベントの解釈ではない（RULE-54）。
  - 再改訂(2026-09-10)。**アクティビティを「一覧」と「詳細画面」に分け、1画面に1つのことだけを出す。** 区切りを開くとその場に一覧と図が広がり、カードの頭に通しの再生も重なる形では、画面のあちこちで動きと文章が同時に変わり、何をしているのか目で追えなかった。一覧のカードは区切りを1行ずつ並べるだけで、何も動かず何も開かない。行の「アニメーションで見る」「ログを読む」とカードの頭の「最初から通して見る」は、同じ `/activity` に `run` / `task` / `view` / `event` を付けたアドレスであり（`ui/activity-links.ts`）、サーバーはアドレスが指す画面をそのまま描画する（`components/activity-viewer.tsx`）。スクリプトが無くてもリンクをたどれば読める点は変わらない。詳細画面は1つの Agent について、アニメーションかログのどちらか一方だけを出し、両者は「いま何手目か」を1つの状態として共有する。手数は区切りの中で数える。アニメーションの説明は図の枠の中の字幕1か所に集め、往復の名前と発行元の `message` に続けて Agent 自身の言葉と実行前の確認だけを添える（`replay/thinking.ts`）。図の横の説明枠は置かない。横と下の2か所に説明があると、どちらを見ればよいか定まらなかったからである。箱を押したときの説明も同じ字幕の場所に出す。ログは区切りのできごとを1行ずつ並べ、選んだ1件の記録だけを開く。`components/run-player.tsx` / `task-replay.tsx` / `stage-card.tsx` / `thinking-panel.tsx` は無くなり、`motion` は進捗の帯にだけ残る。区切りの並び、登場人物の紹介、「問題があったものだけ」（行を隠すだけ）はそのままである。
...
- DEC-APP-08 外部依存は許可リストで固定する。hono / @hono/node-server / ajv / ajv-formats / json-schema-to-ts / yaml / @google-cloud/{firestore,kms,pubsub,secret-manager,storage,bigquery,run,vertexai} と maronn の core / experimental / cli。JWT 検証、JWS 署名、DPoP、JWK Thumbprint、base64url は自前実装する。
  - 追補(2026-09-06、DEC-APP-06 の改訂に伴う)。Automation App の画面のために react / react-dom / motion を、そのテストのために happy-dom を加える。追加はこの3つ+1つだけで、グラフ描画・UI コンポーネント集・状態管理・データ取得のライブラリは入れない。許可リストの実体は `scripts/allowed-deps.json` であり、`pnpm check:deps` が全 package.json を照合する。
