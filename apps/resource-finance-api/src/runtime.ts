...
import type { FinanceApiDeps } from './app.js';
...
export async function createRuntimeDeps(env: NodeJS.ProcessEnv = process.env): Promise<FinanceApiDeps & { port: number }> {
  ...
  return {
    ...
    documents: createFirestoreDocumentStore(firestore, 'resource-finance-api'),
    ...
    absoluteMaxAmount: Number(required(env, 'FINANCE_ABSOLUTE_MAX_AMOUNT')),
  };
}
