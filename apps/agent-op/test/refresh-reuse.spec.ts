...
describe('refresh token reuse', () => {
  it('second use of same refresh token is rejected', async () => {
    ...
    // No new subject token, and nothing new written over the connection.
    expect(await storedConnection(fixture)).toBeTruthy();
    expect(await fakeEnvelope.decrypt((await storedConnection(fixture))!.encrypted_refresh_token, fixture.agentId))
      .toBe(ORIGINAL);
  });
