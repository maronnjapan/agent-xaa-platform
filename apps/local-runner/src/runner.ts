...
import { loadLocalConfig, type LocalRunnerConfig } from './config.js';
...
export async function startLocalPlatform(
  ...
  listeners.push(startLifecycle(platform, run));
  ...
  if (config.seed) await runLocalSeed(platform);
  const tick = startLifecycleTick(platform);
  if (!config.quiet) printBanner(config, listeners);
...
function printBanner(config: LocalRunnerConfig, listeners: readonly Listener[]): void {
  ...
  process.stdout.write([
    ...
    '',
    '  State is in memory: stopping this process discards every agent, ToDo and document.',
    '',
  ].join('\n'));
}
