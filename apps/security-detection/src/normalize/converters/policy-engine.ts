...
export function convert(entry: LogEntry): NormalizedEvent {
  ...
  return {
    ...
    api: {
      operation: String(fields.decision_stage ?? entry.event),
      method: 'POST',
      resource: String(fields.capability_id ?? ''),
      status: String(fields.decision ?? entry.severity),
    },
    metadata: {
