...
/** agent_op_idp_connection: docs 09 §2. */
export function convert(entry: LogEntry): NormalizedEvent {
  const fields = entry.fields;
  return {
    class_uid: CLASS_UID.agent_op_idp_connection,
    activity_id: 6,
    ...
    api: {
      operation: String(fields.connection_operation ?? entry.event),
      method: 'POST',
      resource: String(fields.idp_connection_id ?? ''),
