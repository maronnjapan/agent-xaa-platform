...
discoveryApp.get('/', (c) => {
  ...
  // XAA-PATCH:REQ-08-018 begin
  // RULE-53: the JWK Set is served from the shared Cloud Storage bucket so every
  // verifier reads one document covering all five kid prefixes. The generated value
  // above points at this service's own endpoint and is overridden here.
  ...
  // DEV-09: under the `direct` profile /xaa/token is on a different host from the
  // issuer and is therefore not reachable from this document, so identity chaining is
  // advertised only under `loadbalancer`. Human IdP never redeems an ID-JAG, so
  // authorization_grant_profiles_supported stays absent in both profiles, and no
  // registration_endpoint is advertised (RULE-50: no dynamic client registration).
  const chained: Record<string, unknown> = c.get('issuerProfile') === 'loadbalancer'
    ? { identity_chaining_requested_token_types_supported: ['urn:ietf:params:oauth:token-type:id-jag'] }
    : {};
  Object.assign(metadata, chained);
  // XAA-PATCH:REQ-05-028 end
