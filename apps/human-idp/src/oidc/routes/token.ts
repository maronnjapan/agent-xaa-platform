...
// XAA-PATCH:REQ-05-018 begin
    // RULE-06 / DEC-ID-13 path (3): the Access Token handed to the Control Plane is
    // bound to the caller's key. maronn has no DPoP support, so verification comes
    // from packages/xaa-crypto (DEV-01). `ath` is not required at /token because the
    // client has not been given an Access Token yet.
    const jtiStore = c.get('jtiStore') as JtiStore | undefined;
    let dpopBinding: { status: string; jkt?: string } = { status: 'absent' };
    if (jtiStore) {
      try {
        dpopBinding = await bindDpop({
          // A compact JWS contains no comma, so a joined multi-value header splits
          // cleanly and more than one proof is rejected as a protocol error.
          proofs: (c.req.raw.headers.get('dpop') ?? '').split(',').map((value) => value.trim()).filter(Boolean),
          issuer: config.issuer,
          jtiStore,
...
// XAA-PATCH:REQ-05-018 begin
    // buildAccessTokenPayload's return type carries an index signature, so the
    // confirmation claim is attached without a cast (RFC 9449 §6).
    if (dpopBinding.jkt) accessTokenPayload.cnf = { jkt: dpopBinding.jkt };
// XAA-PATCH:REQ-05-018 end
...
// XAA-PATCH:REQ-05-018 begin
    // RFC 9449 §5: a key-bound token is presented with the DPoP scheme, so the
    // response must say so. Bearer remains only for the unbound path.
    if (dpopBinding.jkt) Object.assign(tokenResponse as Record<string, unknown>, { token_type: 'DPoP' });
// XAA-PATCH:REQ-05-018 end
...
// XAA-PATCH:REQ-05-018 begin
    // Introspection echoes cnf from the stored record, so a resource server that
    // introspects an opaque token sees the same binding as a JWT verifier.
    if (dpopBinding.jkt) {
      const stored = await accessTokenStore.get(tokenResponse.access_token);
      if (stored) await accessTokenStore.set(tokenResponse.access_token, { ...stored, cnf: { jkt: dpopBinding.jkt } });
    }
// XAA-PATCH:REQ-05-018 end
...
// XAA-PATCH:REQ-09-004 begin
      (c.get('auditHooks') as AuditHooks | undefined)?.token(
        { auth_result: 'failure', failure_code: error.error }, c.req.raw,
      );
// XAA-PATCH:REQ-09-004 end
      return c.json(
