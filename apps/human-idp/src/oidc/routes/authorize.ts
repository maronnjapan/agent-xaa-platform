...
import { AuthorizationErrorCode } from '@maronn-openid-connect/core';
import type { AuditHooks } from '../../log/audit-log.js';
import { findUnregisteredScope, UNREGISTERED_SCOPE_DESCRIPTION } from '../../config/scopes.js';
import { isAllowedTarget, INVALID_TARGET_DESCRIPTION } from '../../config/allowed-targets.js';
import { decideAudience } from '../../config/audience-map.js';
import type { OfflineAccessPolicy } from '../../auth/offline-access-policy.js';
...
// XAA-PATCH:REQ-05-004 begin
    // core only checks that `openid` is present. Narrowing to the registered set and
    // to the per-client allow list happens here. An unregistered scope is rejected
    // rather than silently dropped, and the offending value never reaches
    // error_description.
    if (findUnregisteredScope(client.clientId, scope) !== undefined) {
      throw new AuthorizationError(AuthorizationErrorCode.InvalidScope, UNREGISTERED_SCOPE_DESCRIPTION, redirectUri, state);
    }
// XAA-PATCH:REQ-05-004 end
...
// XAA-PATCH:REQ-05-006 begin
    // The generated call below applies core's default policy. RULE-51 needs a wider
    // one for agent-platform, where a recorded consent stands in for prompt=consent,
    // so the pre-policy scope is captured and the policy is re-applied with it.
    const scopeBeforeOfflinePolicy = scope;
// XAA-PATCH:REQ-05-006 end
    scope = await applyOfflineAccessPolicy(scope, effectiveParams, prompt, client);
// XAA-PATCH:REQ-05-006 begin
    const offlineAccessPolicy = c.get('offlineAccessPolicy') as OfflineAccessPolicy | undefined;
    if (offlineAccessPolicy) {
      const cookieHeader = c.req.header('cookie') ?? null;
      scope = await applyOfflineAccessPolicy(
        scopeBeforeOfflinePolicy, effectiveParams, prompt, client, offlineAccessPolicy.forCookie(cookieHeader),
      );
      // A prompt=none request that asked for offline_access while logged in but with
      // no consent record has to say so. Dropping the scope silently would leave the
      // Provisioner unable to tell "no refresh token" apart from "consent needed".
      // With no session at all the answer is login_required, which core produces.
      if (scopeBeforeOfflinePolicy.includes('offline_access')
        && !scope.includes('offline_access')
        && (prompt ?? []).includes('none')
        && await offlineAccessPolicy.hasSession(cookieHeader)) {
        throw new AuthorizationError(AuthorizationErrorCode.InteractionRequired, 'Consent is required before offline_access can be granted', redirectUri, state);
      }
    }
// XAA-PATCH:REQ-05-006 end
...
// XAA-PATCH:REQ-05-003 begin
    // core has no target policy, so the per-client allow list and the
    // scope-to-audience table are applied here. Neither the requested value nor the
    // allow list is echoed back to the caller.
    if (!isAllowedTarget(client.clientId, audience)) {
      return c.redirect(buildErrorRedirect(redirectUri, 'invalid_target', state, INVALID_TARGET_DESCRIPTION, issuer));
    }
    const audienceDecision = decideAudience(scope, audience);
    if (audienceDecision.outcome === 'error') {
      if (audienceDecision.error === 'invalid_scope') {
        throw new AuthorizationError(AuthorizationErrorCode.InvalidScope, UNREGISTERED_SCOPE_DESCRIPTION, redirectUri, state);
      }
      return c.redirect(buildErrorRedirect(redirectUri, 'invalid_target', state, INVALID_TARGET_DESCRIPTION, issuer));
    }
// XAA-PATCH:REQ-05-003 end
...
// XAA-PATCH:REQ-05-003 begin
    // The access token audience follows from the requested operation scope, so the
    // value carried through the transaction is the mapped one, not the raw parameter.
    validatedRequest.audience = audienceDecision.outcome === 'audience' ? [audienceDecision.audience] : undefined;
// XAA-PATCH:REQ-05-003 end
...
// XAA-PATCH:REQ-09-004 begin
    // docs 09 §2: one line per completed /authorize. Emitted here, not from a
    // middleware, so only the three endpoints the design names produce audit lines.
    (c.get('auditHooks') as AuditHooks | undefined)?.authorize(
      { client_id: client.clientId, audience: validatedRequest.audience ?? null, scope: scope.join(' '), auth_result: 'success' },
      c.req.raw,
    );
// XAA-PATCH:REQ-09-004 end
...
// XAA-PATCH:REQ-09-004 begin
        (c.get('auditHooks') as AuditHooks | undefined)?.authorize(
          { client_id: typeof params.client_id === 'string' ? params.client_id : null, scope: typeof params.scope === 'string' ? params.scope : null, auth_result: 'failure', failure_code: error.error },
          c.req.raw,
        );
// XAA-PATCH:REQ-09-004 end
        return c.redirect(redirectUrl.toString());
