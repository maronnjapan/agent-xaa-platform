 ...
 * one-shot helper, because the confirmation binding and the revocation check have to
 * sit between library steps. The order is the property: a check that runs after the
 * token is minted is not a check. REQ-08-044 also depends on it — there is no branch
 * that can skip ahead, so no Cloud Run header can shorten the list.
 */
describe('the redemption pipeline runs in one fixed order', () => {
  ...
  it('has no isolation step at all in the documents pipeline', async () => {
    const chain = await createRedeemableAs();
    await chain.redeem({ assertion: await chain.mint({ isolationLevel: 'standard' }) });
    expect(chain.as.steps).not.toContain('isolation');
  });
