...
describe('trusted identity provider', () => {
  ...
  it('refuses an ID-JAG signed with this AS\'s own key prefix', async () => {
    const chain = await createRedeemableAs({}, {}, 'docs-as-12345678');
  ...
  it('refuses to start without the trusted issuer configured', () => {
    const complete: NodeJS.ProcessEnv = {
      ...
      REGISTERED_SCOPES: 'docs.read docs.write', SIGNING_KEY_BUCKET: 'b', SIGNING_KEY_OBJECT: 'resource-as-signing/docs/current.json', SIGNING_KEY_KMS_KEY: 'k',
  ...
  it('refuses to start when the registered scopes are widened', () => {
    const complete: NodeJS.ProcessEnv = {
      ISSUER: asEnv.issuer, TRUSTED_IDP_ISSUER: asEnv.trustedIdpIssuer, TRUSTED_IDP_JWKS_URI: asEnv.trustedIdpJwksUri,
      REGISTERED_SCOPES: 'docs.read docs.admin', SIGNING_KEY_BUCKET: 'b', SIGNING_KEY_OBJECT: 'resource-as-signing/docs/current.json', SIGNING_KEY_KMS_KEY: 'k',
      JWKS_BUCKET: 'j', JWKS_KEY_PREFIX: 'docs-as', RESOURCE: asEnv.resourceUri,
    };
