...
import { nameOf, type ActorRole } from '../roles.js';
import { CAST_DOES_NOT, RoleCard } from './cast-panel.js';
import { SimulatedBadge } from './simulated-badge.js';
import type { Element } from '../element.js';

export const REPLAY_LEGEND_CAPTION = '図の見方';
...
export const REPLAY_LEGEND: readonly string[] = [
  '箱は「人」「決める側」「動く側」「データを持つ側」の4つの帯に分かれています。',
  '動く丸が1回のやり取りです。線は丸と一緒に伸び、矢印の先が届いた相手です。',
  '止められたやり取りは相手に届く手前で止まり、届かなかった残りは点線になります。',
  '1回の Tool 呼び出しは複数の往復からなり、済んだ往復は薄い線で残ります。',
  '箱の中だけで起きたことは、矢印を描かずに箱が光ります。',
  '箱を押すと、その説明が出ます。関わっていない箱は表示されません。',
];

export const REPLAY_CAPTION_IDLE = '「再生」を押すと、動きに合わせてここに説明が出ます。';
export const ROLE_CLOSE_LABEL = '説明を閉じる';
...
export interface ReplayCanvasProps {
  ...
  /** The part a pressed box stands for; its card takes the caption's place while it is open. */
  openRole?: ActorRole | null;
  /**
  ...
  tools?: ReactNode;
  /** What else the caption says about the step: the agent's own words, the checks it made. */
  notes?: ReactNode;
}
 ...
 * than removed, and everything there is to say about a step written inside the same
 * frame.
 *
 ...
 *
 * The words go under the picture, inside its frame, the way subtitles sit inside a
 * film: the two boxes, the exchange, the publisher's own sentence about it — which
 * appears when the dot lands, not before — and whatever else the step has to say
 * (`notes`). There used to be a second place for words, a panel beside the picture,
 * and a person watching had to choose between the two; now there is one, and the eye
 * never leaves the frame. A pressed box puts its description in the same place. Every
 * word in it comes off the event or the fixed dictionary, never composed (RULE-54).
 *
 * It holds the current step and only the current step, and so does the canvas. What
 * did happen, in order and in full, is the written account on the viewer's other
 * face — server-rendered from the same events, complete, and never wiped by the
 * picture.
 *
 * The controls sit under the frame, where a film's do. They exist because a replay
 * that only ran once, start to finish, at a fixed pace, is a thing you watch rather
 * than a thing you read. A step that says something surprising is the step you want
 * to stop on.
 */
export function ReplayCanvas(props: ReplayCanvasProps): Element {
  ...
  const opened = props.openRole ?? null;
  return (
    <div
      className="replay"
    ...
    >
      <div className="replay-frame" data-replay-frame="true">
        ...
        <div
          ...
          data-caption="true"
          data-caption-state={opened ? 'role' : captionState(props.state, step?.blocked === true)}
          {...(emphasis && !opened ? { 'data-caption-emphasis': emphasis } : {})}
        >
          {opened
            ? (
              <div className="replay-role-open" data-role-open={opened.id}>
                <RoleCard actor={opened} />
                <button type="button" className="secondary" data-action="close-role" onClick={() => props.onOpenNode?.(null)}>
                  {ROLE_CLOSE_LABEL}
                </button>
              </div>
            )
                ...
                {cast
                  ? (
                    <p className="caption-message caption-does-not" data-field="caption-does-not">
                      <span className="caption-note-label">{CAST_DOES_NOT}</span>
                      {cast.doesNot}
                    </p>
                  )
                  : null}
                {step && !cast ? props.notes : null}
              </>
            )}
        </div>
      </div>
