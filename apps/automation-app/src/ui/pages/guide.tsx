  ...
  return (
    <main className="guide" data-page="guide">
      ...
      <section className="card" data-step="operate">
        ...
        <p>起きたことは<a href="/activity">アクティビティ</a>で見ます。Agent ごとに1枚のカードになり、「準備」「作業 1」「終了」の区切りが起きた順に1行ずつ並びます。行の「アニメーションで見る」を押すと、その区切りの動きを図で再生します。説明は図の枠の中に字幕として出ます。「ログを読む」を押すと、できごとを1件ずつ読めます。アニメーションとログは同じ画面で切り替え、一度に出るのはどちらか一方です。</p>
        <p>カードの頭の「最初から通して見る」を押すと、図に出てくる登場人物を1つずつ紹介してから、ログインから終了までを1つの図で通して再生します。人に見せるときはこちらが向いています。「速く」で1手が半分の長さになり、「全画面」で画面いっぱいに出せます。</p>
      ...
      <section className="card" data-step="trouble">
        ...
        <dl>
          ...
          <dd>アクティビティの上にある「問題があったものだけ」を押すと、遮断や失敗のあった区切りだけが残ります。その「ログを読む」を開くと、止められた行に「遮断」の印が付いています。</dd>
          <dt>図の動きが速くて追えない</dt>
          <dd>「一時停止」で止まり、「次へ」で1手ずつ進みます。「ログ」に切り替えると、同じ手の記録を文章で読めます。</dd>
          <dt>箱の名前が分からない</dt>
          <dd>図の箱を押すと、その説明が字幕の場所に出ます。「最初から通して見る」で開くと、最初に登場人物を1つずつ紹介します。</dd>
