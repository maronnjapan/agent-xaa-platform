 ...
 * 全然わからない」.
 *
 * So each part gets four fixed strings: what it is called, the one phrase that goes
 * inside its box, what it does, and what it deliberately does not do. The last one
 * carries most of the weight — this platform's whole point is that the thing which
 * decides is not the thing which acts, and a reader who does not know that cannot see
 * why an arrow stopping short matters.
 *
